...
class LZ78Compressor(Compressor):
    ...
    def compress(self, data: bytes) -> bytes:
        ...
        Алгоритм проходит по входным данным, выделяя наибольшую последовательность, уже присутствующую в словаре.
        Если найденная последовательность w расширяется символом c так, что w+c отсутствует в словаре,
        выводится пара: (индекс(w), c), где для пустой последовательности индекс считается равным 0.
        Если входной поток заканчивается, и остаётся неполная последовательность w, выводится только 2‑байтовый индекс.
    ...
    def decompress(self, data: bytes) -> bytes:
        """
        Восстанавливает исходные данные, декодируя последовательность пар, сформированных алгоритмом LZ78.
